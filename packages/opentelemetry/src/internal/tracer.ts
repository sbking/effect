...
/** @internal */
export const currentSpanContext: Effect.Effect<OtelApi.SpanContext, Cause.NoSuchElementException> = Effect.map(
  Effect.currentSpan,
  (span): OtelApi.SpanContext => {
    if (OtelSpanTypeId in span) {
      return (span as OtelSpan).span.spanContext()
    }
    return {
      traceId: span.traceId,
      spanId: span.spanId,
